...
// The form needs props for the ID of the video its commenting on,
// and a function to refetch comments after submission. (onCommentAdded)
function AddCommentForm({ videoId, onCommentAdded }) {
  ...
  // Getting states by deconstructing the slice
  ...
  async function handleSubmit(e) {
    ...
    // Logic to call the protected POST /api/video/:id/comment API
    try {
      ...
      const response = await axios.post(
        ...
        { text: commentText },
        // Configuration object: Must include the Authorization header with the JWT token.
        {
          headers: {
    ...
    return (
      <div className="flex justify-start items-center p-3 my-4 bg-gray-50 border rounded-lg">
        ...
        <p className="text-sm text-gray-700">
          Please <span className="font-bold text-red-600">Sign In</span> to post
          a comment.
        </p>
      </div>
    );
  ...
  return (
    <div className="flex space-x-3 my-4">
      ...
      <FaUserCircle className="text-4xl text-gray-500 shrink-0 mt-1" />
      ...
      <form onSubmit={handleSubmit} className="w-full">
        ...
        {/* using length to check if text is entered */}
